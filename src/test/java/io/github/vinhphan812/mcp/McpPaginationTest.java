...
class McpPaginationTest {
    ...
    void toolsListReturnsFirstAndNextPage() {
        ...
        registry.registerTool("one", "one", Collections.emptyMap(), Collections.emptyList(),
                p -> Collections.emptyMap());
        registry.registerTool("two", "two", Collections.emptyMap(), Collections.emptyList(),
                p -> Collections.emptyMap());
        McpProtocolHandler handler = new McpProtocolHandler(registry, McpServerConfig.builder().pageSize(1).build());
        String session = handler
                .handleRequestResponse("{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{}}", null)
                .getSessionId();

        JsonObject first = parse(
                handler.handleRequest("{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/list\"}", session));
        ...
        JsonObject second = parse(handler.handleRequest(
                "{\"jsonrpc\":\"2.0\",\"id\":3,\"method\":\"tools/list\",\"params\":{\"cursor\":\"" + cursor + "\"}}",
                session));
        assertEquals("two", second.getAsJsonObject("result").getAsJsonArray("tools").get(0).getAsJsonObject()
    ...
    void resourcesListRejectsInvalidCursor() {
        ...
        String response = handler.handleRequest(
                "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"resources/list\",\"params\":{\"cursor\":\"bad\"}}",
                session);
