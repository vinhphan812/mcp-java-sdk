...
| DOC-006 | RateLimits.Builder missing sessionTimeoutMs, sessionCleanupIntervalMs, maxPendingNotificationsPerSession, maxRequestsPerSessionPerMinute, rateLimitWindows, overflowPolicy | **OPEN** | `API-REFERENCE.md` has **no RateLimits section**; all six methods and the `QueueOverflowPolicy` enum are absent. This is a documentation gap, not a code gap. Priority: add RateLimits section. ||
...
| DOC-010 | apiKeyStore/apiKeyMiddleware request-pipeline impact removed                                                                                                               | **OPEN** | `McpServerConfig.Builder` confirmed absent of `apiKeyStore`/`apiKeyMiddleware` fields. Examples compile without calling them. Priority: update evidence to reflect current state.               ||
...
| DOC-012 | QueueOverflowPolicy enum not documented in API-REFERENCE.md                                                                                                                | **OPEN** | `API-REFERENCE.md` has **no RateLimits section**; `QueueOverflowPolicy` enum is absent. Covered by DOC-006 fix.                                                                                 ||
| DOC-013 | Default destructive-tool registrations not documented                                                                                                                      | **OPEN** | `API-REFERENCE.md` has **no RateLimits section**; default destructive tool registrations (shutdown, delete_action, etc.) are absent. Covered by DOC-006 fix.                                    |
